...
--        收盤跌破 20MA 才出、−8% 停損、最長 60 交易日。讓利潤奔跑。
-- 與 strat-5-10-20 共用進場、差在出場 → 換取大波段，但報酬集中少數肥尾單、勝率低、變異大。
-- 回測(5槽位2016-26 in-sample)：報酬遠高於 5-10-20 bracket，但勝率 ~32%、靠 ~10 筆沿軌大單，需留意 OOS。
-- ★ 評分不混用 bracket：見 evaluate_bb_trend()（跌破20MA/−8%/60日）；主 evaluate 已排除本 skill。
...
VALUES ('strat-bb-trend', 1, 'champion', 'ALL',
  jsonb_build_object('entry','strat-5-10-20 buy','exit','trail_ma20',
                     'stop_pct',0.08,'maxhold',60,'horizon_days',5),
  'seed-bbtrend-v1','human',
  '布林通道趨勢續抱：5-10-20進場 + 跌破20MA出場(讓利潤奔跑)。報酬高但集中少數大波段、勝率低、變異大。')
ON CONFLICT (family, version) DO NOTHING;

-- 最新買訊（同 5-10-20 buy）+ 趨勢續抱交易計畫（停損 −8%、出場=跌破20MA、無固定獲利目標）
CREATE OR REPLACE VIEW v_bb_trend_latest AS
SELECT symbol, ts, score, signal_type, close AS entry_price,
       round(close*0.92, 2) AS stop_price, '跌破20MA 出場' AS exit_rule
...
BEGIN
  ...
  SELECT l.symbol, 'strat-bb-trend', v_skill_id, l.ts, 5,
    l.ts + make_interval(days => 84),         -- 約 60 交易日 maxhold 後可結算
    'long', 'up', l.score, l.signal_type, l.close,
    jsonb_build_object('exit_type','trail_ma20','stop_pct',0.08,'maxhold',60)
...
-- 專屬評分：對 strat-bb-trend 未評分預測，用「跌破20MA / −8% / 60日」結算（非 bracket）
CREATE OR REPLACE FUNCTION evaluate_bb_trend(p_maxhold INT DEFAULT 60, p_cost NUMERIC DEFAULT 0.006)
RETURNS INT LANGUAGE plpgsql AS $$
DECLARE n INT;
BEGIN
  ...
  CROSS JOIN LATERAL (
    WITH fwd AS (
      SELECT i.rn, i.ts, i.cl, (i.lo <= e.cl*0.92) AS sl,
             (i.cl < m.ma20) AS brk
      FROM _px i JOIN _m m ON m.symbol=i.symbol AND m.ts=i.ts
      WHERE i.symbol=a.symbol AND i.rn>e.rn AND i.rn<=e.rn+p_maxhold
    ),
    fh AS (SELECT CASE WHEN sl THEN round(e.cl*0.92,4) ELSE cl END exit_price,
                  CASE WHEN sl THEN 'stop' ELSE 'ma20_break' END reason
           FROM fwd WHERE sl OR brk ORDER BY rn LIMIT 1),
    mt AS (SELECT cl exit_price, 'maxhold' reason FROM _px WHERE symbol=a.symbol AND rn=e.rn+p_maxhold)
    SELECT exit_price, reason FROM fh
    UNION ALL SELECT exit_price, reason FROM mt WHERE NOT EXISTS(SELECT 1 FROM fh) LIMIT 1
  ) x
